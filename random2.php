...
<body>
	...
	for ($zeile = 1; $zeile < 27; $zeile++){ 			// schleife für zeilen fortschritt
		...
		for ($spalte = 1; $spalte < 40; $spalte++){	// schleife für die spalten
			...
			$farbe2 = rand(0,127);
			$farbe3 = rand(0,127);
			$zufallrot = $farbe1 + $farbe2; 				//zufallswert für rot
			$zufallgrun = $farbe1 + $farbe3; 			//zufallswert für grun
			$zufallblau = $farbe2 + $farbe3; 				//zufallswert für blau
			$pos = $positionx."px";
			if ($zeile % 2 != 0){
				...
				$div = $div."<div class='box' style='background:rgba($action2,$action3,$action1,1);  
